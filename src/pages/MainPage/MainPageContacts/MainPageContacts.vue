...
import DoctorIcon from '../../../assets/Logo.svg'
...
<template>
  <VContainer>
    <VCard>
      <VCardText>
        <VTabs v-model="currentTab" align-tabs="center" mandatory="force">
          <VTab v-for="tab in locations" :key="tab.value" :text="tab.title" :value="tab.value" />
        </VTabs>
        <VTabsWindow v-model="currentTab" class="mt-4">
          <VTabsWindowItem :value="location.value" v-for="location in locations" :key="location.value">
            <VRow class="d-flex align-center">
              <VCol cols="12" order="2" order-sm="1" md="7">
                <div style="position: relative" class="pa-2">
                  <iframe
                    :src="location.mapIFrameLink"
                    :width="mapWidth"
                    height="400"
                    frameborder="0"
